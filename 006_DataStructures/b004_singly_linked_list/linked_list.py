...
# Did a little change about the parameters in the function, because didn't know if the function should be in the class
# Will rewrite it if needed

def sum_two_lists(llist, lllist):
    new_list = LinkedList()

    first_node = llist.head
    second_node = lllist.head

    while first_node or second_node:
        if first_node and second_node:
            if first_node.data + second_node.data >= 10:
                new_list.append((first_node.data + second_node.data) % 10)
                first_node.next.data = first_node.next.data + 1
            else:
                new_list.append(first_node.data + second_node.data)

            first_node = first_node.next
            second_node = second_node.next
        else:
            if first_node is None:
                new_list.append(0 + second_node.data)
                second_node = second_node.next
            else:
                new_list.append(0 + first_node.data)
